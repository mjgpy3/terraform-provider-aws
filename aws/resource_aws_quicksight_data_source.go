...
func resourceAwsQuickSightDataSourceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("tags"); ok {
		params.Tags = keyvaluetags.New(v.(map[string]interface{})).IgnoreAws().QuicksightTags()
	}
	...
	createOut, err := conn.CreateDataSource(params)
	if err != nil {
		return fmt.Errorf("Error creating QuickSight Data Source: %s", err)
	} else {
		_, err = waitQuickSightDataSourceCreate(conn, awsAccountId, dataSourceId, d.Timeout(schema.TimeoutCreate))
		if err != nil {
			return fmt.Errorf("Error waiting for Data Source (%s) to become available: %s", dataSourceId, err)
		}

		d.SetId(fmt.Sprintf("%s/%s", awsAccountId, dataSourceId))
		d.Set("arn", createOut.Arn)
...
func waitQuickSightDataSourceCreate(conn *quicksight.QuickSight, awsAccountId string, id string, timeout time.Duration) (interface{}, error) {
	stateChangeConf := &resource.StateChangeConf{
		...
		Refresh: quickSightDataSourceStateRefreshFunc(conn, awsAccountId, id),
		Timeout: timeout,
		Delay:   5 * time.Second,
	}
...
func waitQuickSightDataSourceRead(conn *quicksight.QuickSight, awsAccountId string, id string, timeout time.Duration) (interface{}, error) {
	stateChangeConf := &resource.StateChangeConf{
		Pending: []string{quicksight.ResourceStatusCreationInProgress, quicksight.ResourceStatusUpdateInProgress},
		Target:  []string{quicksight.ResourceStatusCreationSuccessful, quicksight.ResourceStatusUpdateSuccessful},
		Refresh: quickSightDataSourceStateRefreshFunc(conn, awsAccountId, id),
		Timeout: timeout,
		Delay:   5 * time.Second,
	}
...
func quickSightDataSourceStateRefreshFunc(conn *quicksight.QuickSight, awsAccountId, id string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		req := &quicksight.DescribeDataSourceInput{
			AwsAccountId: aws.String(awsAccountId),
			DataSourceId: aws.String(id),
		}

		resp, err := conn.DescribeDataSource(req)
		if err != nil {
			return nil, "", err
		}
		...
		state := ""
		if aws.StringValue(resp.DataSource.Status) == quicksight.ResourceStatusCreationSuccessful {
			state = quicksight.ResourceStatusCreationSuccessful
			log.Printf("resp.DataSource.Status %#v", aws.StringValue(resp.DataSource.Status))
		}
		if aws.StringValue(resp.DataSource.Status) == quicksight.ResourceStatusUpdateSuccessful {
			state = quicksight.ResourceStatusUpdateSuccessful
			log.Printf("resp.DataSource.Status %#v", aws.StringValue(resp.DataSource.Status))
		}

		return dataSourceId, state, nil
	}
}
...
func resourceAwsQuickSightDataSourceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		_, err = waitQuickSightDataSourceRead(conn, awsAccountId, dataSourceId, d.Timeout(schema.TimeoutRead))
		if err != nil {
			return fmt.Errorf("Error waiting for Data Source (%s) to become available: %s", dataSourceId, err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("error listing tags for AWS QuickSight resource (%s): %s", d.Get("arn").(string), err)
	}
	err = d.Set("tags", tags.IgnoreAws().Map())
	if err != nil {
...
func resourceAwsQuickSightDataSourceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("tags") {
		...
		err := keyvaluetags.QuicksightUpdateTags(conn, d.Get("arn").(string), o, n)
		if err != nil {
			return fmt.Errorf("error updating AWS QuickSight resource (%s) tags: %s", d.Get("arn").(string), err)
		}
	}
